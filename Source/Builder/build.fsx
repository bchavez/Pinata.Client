...
let ProjectName = "Pinata.Client";
let GitHubUrl = "https://github.com/bchavez/Pinata.Client"

let Folders = Setup.Folders(workingDir)
let Files = Setup.Files(ProjectName, Folders)

let PinataClientProject = NugetProject("Pinata.Client", "Pinata API for .NET", Folders)
let TestProject = TestProject("Pinata.Client.Tests", Folders)
...
Target.create "dnx" (fun _ ->
    ...
    DotNet.build releaseConfig PinataClientProject.Folder
    DotNet.build debugConfig PinataClientProject.Folder

    Shell.copyDir PinataClientProject.OutputDirectory (PinataClientProject.Folder @@ "bin") FileFilter.allFiles
...
Target.create "restore" (fun _ -> 
     ...
     DotNet.restore id PinataClientProject.Folder
...
Target.create "nuget" (fun _ ->
    ...
    DotNet.pack config PinataClientProject.Folder
)
...
Target.create "zip" (fun _ -> 
    ...
    !!(PinataClientProject.OutputDirectory @@ "**")
    |> Zip.zip Folders.CompileOutput (Folders.Package @@ PinataClientProject.Zip)
)
...
Target.create "BuildInfo" (fun _ ->
    ...
    MakeBuildInfo PinataClientProject Folders (fun bip -> { bip with ExtraAttrs = customAttributes } )

    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/Version" BuildContext.FullVersion

    let releaseNotes = History.NugetText Files.History GitHubUrl
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/PackageReleaseNotes" releaseNotes
)
...
Target.create "Clean" (fun _ ->
    ...
    let projects = [PinataClientProject.Folder; TestProject.Folder;]

    for project in projects do
      Shell.cleanDir (project @@ "bin")
      Shell.cleanDir (project @@ "obj")

    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/Version" "0.0.0-localbuild"
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/PackageReleaseNotes" ""
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/AssemblyOriginatorKeyFile" ""
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/SignAssembly" "false"
    ...
    MakeBuildInfo PinataClientProject Folders (fun bip ->
         {bip with
...
Target.create "setup-snk"(fun _ ->
    ...
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/AssemblyOriginatorKeyFile" Files.SnkFile
    Xml.pokeInnerText PinataClientProject.ProjectFile "/Project/PropertyGroup/SignAssembly" "true"
